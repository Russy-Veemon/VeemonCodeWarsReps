...
def expand(expr):
    # extract the values of a, b, and n from the input expression
    a, b, n = parse_expression(expr)
    
    # compute the coefficients and powers of each term in the expansion
    terms = []
    for i in range(n+1):
        coef = binomial_coefficient(n, i) * a**(n-i) * b**i
        power = n-i
        if coef != 0:
            terms.append(format_term(coef, power))
    
    # join the terms to obtain the final result
    return '+'.join(terms)
